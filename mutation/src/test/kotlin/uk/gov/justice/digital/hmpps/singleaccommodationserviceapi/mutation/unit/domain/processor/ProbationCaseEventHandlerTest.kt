...
class ProbationCaseEventHandlerTest {
  ...
  fun `partition key is CRN`() {
    ...
    assertThat(probationCaseEventHandler.getPartitionKey(inboxEvent)).isEqualTo(crn)
  }
  ...
  @Test
  fun `should not refresh case and ignore probation case message when case is not known`() {
