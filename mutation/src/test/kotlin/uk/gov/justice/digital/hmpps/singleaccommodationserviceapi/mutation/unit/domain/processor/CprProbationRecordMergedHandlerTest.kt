...
import uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.mutation.domain.processor.handler.CprProbationRecordMergedHandler
import java.util.UUID

@ExtendWith(MockKExtension::class)
class CprProbationRecordMergedHandlerTest {
  ...
  @InjectMockKs
  private lateinit var cprProbationRecordMergedHandler: CprProbationRecordMergedHandler
  ...
  fun `should refresh case and process message when case is known`() {
    ...
    assertThat(cprProbationRecordMergedHandler.handle(inboxEvent)).isEqualTo(InboxEventHandler.Result.PROCESSED)
  ...
  fun `should not refresh case and ignore message when case is not known`() {
    ...
    assertThat(cprProbationRecordMergedHandler.handle(inboxEvent)).isEqualTo(InboxEventHandler.Result.IGNORED)
  ...
  fun `should not refresh case and should process message when case refresh request service is null`() {
    cprProbationRecordMergedHandler =
      CprProbationRecordMergedHandler(caseRepository, inboxEventHelper, null)
