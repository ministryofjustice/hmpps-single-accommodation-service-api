...
class OutOfRegionReferralAggregateTest {
  ...
  fun `create with SUBMITTED status should set all fields`() {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
  ...
  fun `create should throw OutOfRegionReferralInvalidStatusException when status is not SUBMITTED`(status: OorStatus) {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)

    assertThrows<OutOfRegionReferralInvalidStatusException> {
      aggregate.updateOutOfRegionReferral(
        submissionDate = submissionDate,
  ...
  fun `update should change status`(currentStatus: OorStatus, newStatus: OorStatus) {
    val aggregate = hydrateAndCreateOutOfRegionReferral(currentStatus)
  ...
  fun `update should keep status when unchanged`(status: OorStatus) {
    val aggregate = hydrateAndCreateOutOfRegionReferral(status)
  ...
  @ParameterizedTest
  @EnumSource(value = OorStatus::class, names = ["ACCEPTED", "NOT_ACCEPTED"])
  fun `update should throw OutOfRegionReferralInvalidStatusTransitionException when reverting to SUBMITTED`(currentStatus: OorStatus) {
    val aggregate = hydrateAndCreateOutOfRegionReferral(currentStatus)

    assertThrows<OutOfRegionReferralInvalidStatusTransitionException> {
      aggregate.updateOutOfRegionReferral(
        submissionDate = submissionDate,
        referenceNumber = null,
        status = OorStatus.SUBMITTED,
  ...
  fun `should addNote successfully`() {
    val aggregate = hydrateAndCreateOutOfRegionReferral(OorStatus.SUBMITTED)
    val note = "note"
  ...
  inner class OutcomeReasonValidation {
    ...
    @ParameterizedTest
    @EnumSource(value = OorStatus::class, names = ["ACCEPTED", "NOT_ACCEPTED"])
    fun `update to ACCEPTED or NOT_ACCEPTED without an outcome reason should throw OutOfRegionReferralOutcomeReasonRequiredException`(status: OorStatus) {
      val aggregate = hydrateAndCreateOutOfRegionReferral(OorStatus.SUBMITTED)
      assertThrows<OutOfRegionReferralOutcomeReasonRequiredException> {
        aggregate.updateOutOfRegionReferral(
          submissionDate = submissionDate,
          referenceNumber = null,
          status = status,
    ...
    @Test
    fun `update to ACCEPTED with a NOT_ACCEPTED outcome reason should throw OutOfRegionReferralOutcomeReasonNotApplicableException`() {
      val aggregate = hydrateAndCreateOutOfRegionReferral(OorStatus.SUBMITTED)
      assertThrows<OutOfRegionReferralOutcomeReasonNotApplicableException> {
        aggregate.updateOutOfRegionReferral(
          submissionDate = submissionDate,
          referenceNumber = null,
          status = OorStatus.ACCEPTED,
    ...
    @Test
    fun `update to NOT_ACCEPTED with an ACCEPTED outcome reason should throw OutOfRegionReferralOutcomeReasonNotApplicableException`() {
      val aggregate = hydrateAndCreateOutOfRegionReferral(OorStatus.SUBMITTED)
      assertThrows<OutOfRegionReferralOutcomeReasonNotApplicableException> {
        aggregate.updateOutOfRegionReferral(
          submissionDate = submissionDate,
    ...
    @Test
    fun `update to SUBMITTED with an outcome reason should throw OutOfRegionReferralOutcomeReasonNotApplicableException`() {
      val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
      assertThrows<OutOfRegionReferralOutcomeReasonNotApplicableException> {
        aggregate.updateOutOfRegionReferral(
          submissionDate = submissionDate,
  ...
  private fun hydrateAndCreateOutOfRegionReferral(status: OorStatus): OutOfRegionReferralAggregate {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
    aggregate.updateOutOfRegionReferral(
      submissionDate = submissionDate,
      referenceNumber = null,
  ...
  @Test
  fun `updateOutOfRegionReferral should throw NoteIsGreaterThanMaxLengthException when submissionNote exceeds 4000 chars`() {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
  ...
  @Test
  fun `updateOutOfRegionReferral should throw NoteIsGreaterThanMaxLengthException when outcomeNote exceeds 4000 chars`() {
    val aggregate = hydrateAndCreateOutOfRegionReferral(OorStatus.SUBMITTED)
  ...
  @ParameterizedTest
  @ValueSource(strings = ["", " ", "   ", "\t", "\n"])
  fun `updateOutOfRegionReferral should ignore blank submissionNote`(note: String) {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
    aggregate.updateOutOfRegionReferral(
      submissionDate = submissionDate,
      referenceNumber = null,
  ...
  @ParameterizedTest
  @ValueSource(strings = ["", " ", "   ", "\t", "\n"])
  fun `updateOutOfRegionReferral should ignore blank outcomeNote`(note: String) {
    val aggregate = hydrateAndCreateOutOfRegionReferral(OorStatus.SUBMITTED)
    aggregate.updateOutOfRegionReferral(
      submissionDate = submissionDate,
      referenceNumber = null,
  ...
  @ParameterizedTest
  @ValueSource(strings = ["", " ", "   ", "\t", "\n"])
  fun `updateOutOfRegionReferral should clear an existing submissionNote when a blank submissionNote is provided on SUBMITTED`(note: String) {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
    aggregate.updateOutOfRegionReferral(
      submissionDate = submissionDate,
      referenceNumber = null,
  ...
  @ParameterizedTest
  @ValueSource(strings = ["", " ", "   ", "\t", "\n"])
  fun `updateOutOfRegionReferral should clear an existing outcomeNote when a blank outcomeNote is provided on an outcome status`(note: String) {
    val aggregate = hydrateAndCreateOutOfRegionReferral(OorStatus.SUBMITTED)
    aggregate.updateOutOfRegionReferral(
      submissionDate = submissionDate,
      referenceNumber = null,
  ...
  @Test
  fun `updateOutOfRegionReferral should replace an existing submissionNote with a new submissionNote on SUBMITTED`() {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
    aggregate.updateOutOfRegionReferral(
      submissionDate = submissionDate,
      referenceNumber = null,
  ...
  @Test
  fun `updateOutOfRegionReferral should update an existing submissionNote on an outcome status`() {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
    aggregate.updateOutOfRegionReferral(
      submissionDate = submissionDate,
      referenceNumber = null,
  ...
  @ParameterizedTest
  @ValueSource(strings = ["", " ", "   ", "\t", "\n"])
  fun `updateOutOfRegionReferral should clear an existing submissionNote when a blank submissionNote is provided on an outcome status`(note: String) {
    val aggregate = OutOfRegionReferralAggregate.hydrateNew(caseId)
    aggregate.updateOutOfRegionReferral(
      submissionDate = submissionDate,
      referenceNumber = null,
  ...
  @Test
  fun `updateOutOfRegionReferral should throw OutOfRegionReferralOutcomeNoteNotApplicableException when outcomeNote provided for SUBMITTED`() {
    val aggregate = hydrateAndCreateOutOfRegionReferral(OorStatus.SUBMITTED)
    assertThrows<OutOfRegionReferralOutcomeNoteNotApplicableException> {
      aggregate.updateOutOfRegionReferral(
        submissionDate = submissionDate,
        referenceNumber = null,
        status = OorStatus.SUBMITTED,
