...
import uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.mutation.factories.buildOutOfRegionReferralNote
import uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.mutation.factories.buildOutOfRegionReferralSnapshot
...
class OutOfRegionReferralMapperTest {

  @Test
  fun `toEntity maps all fields correctly`() {
    val snapshot = buildOutOfRegionReferralSnapshot()

    val entity = OutOfRegionReferralMapper.toEntity(snapshot)
  ...
  fun `toEntity maps status enum values correctly`(
    ...
    val entity = OutOfRegionReferralMapper.toEntity(
      snapshot = buildOutOfRegionReferralSnapshot(status = status),
    )
  ...
  fun `toDto maps all fields correctly and sets status from snapshot`() {
    val snapshot = buildOutOfRegionReferralSnapshot(status = OorStatus.ACCEPTED)
    val createdBy = "Joe Bloggs"
    val createdAt = Instant.now()
    var crn = UUID.randomUUID().toString()

    val dto = OutOfRegionReferralMapper.toDto(snapshot, crn, createdBy, createdAt)
  ...
  fun merge() {
    ...
    val preExistingNoteEntity = buildOutOfRegionReferralNoteEntity(
      id = UUID.randomUUID(),
      note = "1111",
      outOfRegionReferralEntity = entity,
    )
    entity.apply {
      notes.add(preExistingNoteEntity)
    }
    val newNote1 = buildOutOfRegionReferralNote(id = UUID.randomUUID(), note = "2222")
    val newNote2 = buildOutOfRegionReferralNote(id = UUID.randomUUID(), note = "3333")
    val preExistingNote = buildOutOfRegionReferralNote(id = preExistingNoteEntity.id, note = preExistingNoteEntity.note)
    val snapshot = buildOutOfRegionReferralSnapshot(
      notes = mutableListOf(newNote1, newNote2, preExistingNote),
    )
    val merged = OutOfRegionReferralMapper.merge(snapshot, entity)
  ...
  fun `toAggregate maps all fields correctly`() {
    ...
    val noteEntity = buildOutOfRegionReferralNoteEntity(
      id = UUID.randomUUID(),
      note = "1111",
      outOfRegionReferralEntity = entity,
    )
    val noteEntity2 = buildOutOfRegionReferralNoteEntity(
      id = UUID.randomUUID(),
      note = "2222",
      outOfRegionReferralEntity = entity,
    )
  ...
  fun `toAggregate maps status enum values correctly`(
    ...
    val entity = buildOutOfRegionReferralEntity(status = status)
  ...
  fun `toDto maps outcome reason onto submission`() {
    ...
    val dto = OutOfRegionReferralMapper.toDto(snapshot, "X123456", "Test User", Instant.now())
  ...
  fun `toEntity maps null submissionNote and outcomeNote correctly`() {
    val snapshot = buildOutOfRegionReferralSnapshot(submissionNote = null, outcomeNote = null)
  ...
  fun `merge copies submissionNote and outcomeNote from snapshot to entity`() {
    val entity = buildOutOfRegionReferralEntity(submissionNote = null, outcomeNote = null)
    val snapshot = buildOutOfRegionReferralSnapshot(
      submissionNote = "Submission note",
      outcomeNote = "Outcome note",
    )
  ...
  fun `merge preserves existing submissionNote when outcomeNote is set`() {
    val entity = buildOutOfRegionReferralEntity(submissionNote = "Original submission note", outcomeNote = null)
    val snapshot = buildOutOfRegionReferralSnapshot(
