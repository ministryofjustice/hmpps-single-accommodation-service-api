...
class CaseAggregateTest {
  ...
  @Test
  fun `updateIdentifiers() should update caseIdentifiers`() {
    val aggregate = CaseAggregate.createNew(
      id = id,
      caseIdentifiers = caseIdentifiers,
    )
    ...
    val newIdentifiers = setOf("NEWCRN" to IdentifierType.CRN, "NEWPRISONNUMBER" to IdentifierType.PRISON_NUMBER)

    aggregate.updateIdentifiers(newIdentifiers)
