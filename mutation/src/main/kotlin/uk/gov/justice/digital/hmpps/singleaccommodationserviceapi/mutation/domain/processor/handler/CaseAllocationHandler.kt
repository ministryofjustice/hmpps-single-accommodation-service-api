  ...
  override fun handle(inboxEvent: InboxEventEntity) {
    ...
    try {
      ...
      if (onboardedTeamsCodes.contains(case.manager.team.code)) {
        ...
        updateInboxEventStatusAndSave(inboxEvent, status = ProcessedStatus.PROCESSED)
      } else {
        updateInboxEventStatusAndSave(inboxEvent, status = ProcessedStatus.NOT_PROCESSED)
      }
