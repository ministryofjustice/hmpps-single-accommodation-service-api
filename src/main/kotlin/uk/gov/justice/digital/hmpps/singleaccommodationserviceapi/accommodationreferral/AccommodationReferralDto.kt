...
enum class CasReferralStatus {
  ...
  companion object {
    fun from(status: Cas1AssessmentStatus): CasReferralStatus = when (status) {
      ...
      Cas1AssessmentStatus.awaitingResponse,
      Cas1AssessmentStatus.inProgress,
      Cas1AssessmentStatus.notStarted,
      -> PENDING
    }

    fun from(status: TemporaryAccommodationAssessmentStatus): CasReferralStatus = when (status) {
      TemporaryAccommodationAssessmentStatus.readyToPlace -> ACCEPTED
      ...
      TemporaryAccommodationAssessmentStatus.inReview,
      -> PENDING
    }
