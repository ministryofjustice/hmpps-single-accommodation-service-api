...
class MockCaseController {
  ...
  fun getMockCases(): ResponseEntity<List<CaseDto>> = ResponseEntity.ok(getMockedCases())

  @PreAuthorize("hasRole('ROLE_PROBATION')")
  @GetMapping("/mock/cases/{crn}")
  fun getMockCase(@PathVariable crn: String): ResponseEntity<List<CaseDto>> = ResponseEntity.ok(
    getMockedCases()
      .filter { it.crn == crn },
  )
}
