...
class FemaleRiskRuleTest {
  ...
  fun `candidate is female and within tiers A3-C3`(tier: String) {
    val data = DomainData(
      tier = tier,
      sex = female,
    )
  ...
  fun `candidate is female and outside tiers A3-C3`(tier: String) {
    val data = DomainData(
      tier = tier,
      sex = female,
    )
  ...
  fun `rule has correct description`() {
    ...
    assertThat(result).isEqualTo("Is Female and is Tier A3 - C3")
  }
}
