...
class ProposedAccommodationDeliusSyncIT : IntegrationTestBase() {
  ...
  @Test
  fun `should get empty list when get proposed-accommodations by crn when there are two 'Unconfirmed' SAS Proposed Accommodations only with no postcodes - no sync required`() {
  ...
  @Test
  fun `should get empty list when get proposed-accommodations by crn when there are two 'Unconfirmed' SAS Proposed Accommodations only with end dates in the past or today - no sync required`() {
    val cprAccommodations = buildCorePersonRecord(
      identifiers = buildIdentifiers(crns = listOf(crn), prisonNumbers = listOf("PRI1")),
      addresses = emptyList(),
    )
