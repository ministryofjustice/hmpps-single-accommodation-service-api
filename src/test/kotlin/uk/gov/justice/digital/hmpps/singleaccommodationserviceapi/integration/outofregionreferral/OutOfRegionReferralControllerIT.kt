package uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.integration.outofregionreferral
...
import uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.integration.outofregionreferral.json.createOorRequestBody
import uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.integration.outofregionreferral.json.expectedGetOorResponseBody
import uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.integration.outofregionreferral.json.expectedGetOutOfRegionReferralTimelineResponse
import uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.integration.outofregionreferral.json.expectedOorResponseBody
import uk.gov.justice.digital.hmpps.singleaccommodationserviceapi.integration.outofregionreferral.json.oorNoteRequestBody
...
class OutOfRegionReferralControllerIT : IntegrationTestBase() {

  @Autowired
  private lateinit var outOfRegionReferralRepository: OutOfRegionReferralRepository
  ...
  fun setup() {
    ...
    databaseUtils.truncate(
      ...
      SasTables.OUT_OF_REGION_REFERRAL,
    )
  ...
  @Test
  fun `should get out of region referral by id with ADDA role`() {
    val entity = outOfRegionReferralRepository.save(
      buildOutOfRegionReferralEntity(
        caseId = case.id,
    ...
    restTestClient.get().uri("/out-of-region-referrals/{id}", entity.id)
  ...
  @Test
  fun `should return 404 when out of region referral not found for ADDA role`() {
    val nonExistentId = UUID.randomUUID()

    restTestClient.get().uri("/out-of-region-referrals/{id}", nonExistentId)
  ...
  @Test
  fun `should get out of region referral by crn and id`() {
    val entity = outOfRegionReferralRepository.save(
      buildOutOfRegionReferralEntity(
        caseId = case.id,
        referenceNumber = "OOR-REF-001",
    ...
    restTestClient.get().uri("/cases/{crn}/out-of-region-referral/{id}", crn, entity.id)
      .withDeliusUserJwt()
      .exchangeSuccessfully()
      .expectBody<String>()
      .value {
  ...
  @Test
  fun `should return 404 when out of region referral not found by crn and id`() {
    val nonExistentId = UUID.randomUUID()

    restTestClient.get().uri("/cases/{crn}/out-of-region-referral/{id}", crn, nonExistentId)
      .withDeliusUserJwt()
      .exchange()
      .expectStatus().isNotFound
  }
  ...
  fun `should return 404 when crn does not match for GET by crn and id`() {
    ...
    restTestClient.get().uri("/cases/{crn}/out-of-region-referral/{id}", "OTHERCRN", entity.id)
      .withDeliusUserJwt()
      .exchange()
      .expectStatus().isNotFound
  }
  ...
  @Test
  fun `should create out of region`() {
    val result = restTestClient.post().uri("/cases/$crn/out-of-region-referral")
    ...
    val persistedRecord = outOfRegionReferralRepository.findByCaseId(case.id)!!
    assertPersistedOutOfRegionReferral(persistedRecord)
  ...
  @Test
  fun `should update out of region referral and return 200 with updated data`() {
    val existingEntity = outOfRegionReferralRepository.save(
      buildOutOfRegionReferralEntity(
        caseId = case.id,
        referenceNumber = "OOR-REF-001",
        submissionDate = LocalDate.of(2026, 1, 15),
    ...
    val result = restTestClient.put().uri("/cases/$crn/out-of-region-referral/${existingEntity.id}")
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        createOorRequestBody(
  ...
  @Test
  fun `should return 404 when updating nonexistent out of region referral`() {
    val nonExistentId = UUID.randomUUID()

    restTestClient.put().uri("/cases/$crn/out-of-region-referral/$nonExistentId")
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        createOorRequestBody(
  ...
  @Test
  fun `should return 404 when updating OOR with CRN that does not match out of region referral`() {
    val existingEntity = createOor()

    restTestClient.put().uri("/cases/OTHERCRN/oor/${existingEntity.id}")
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        createOorRequestBody(
          status = EntityOorStatus.ACCEPTED.name,
  ...
  fun `should return 400 when accepting OOR referral without an outcome reason`() {
    ...
    restTestClient.put().uri("/cases/$crn/out-of-region-referral/${existingEntity.id}")
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        createOorRequestBody(
          status = EntityOorStatus.ACCEPTED.name,
  ...
  @Test
  fun `should update out of region when status stays the same`() {
    val existingEntity = createOor()
    ...
    val updatedRecord = outOfRegionReferralRepository.findById(existingEntity.id).get()
  ...
  fun `should create a note for oor`() {
    ...
    restTestClient.post().uri("/cases/$crn/out-of-region-referral/${existingEntity.id}/notes")
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        oorNoteRequestBody(
          note = note1Value,
    ...
    var oorPersistedResult = outOfRegionReferralRepository.findByIdAndCrnWithNotes(existingEntity.id, crn)!!
    ...
    oorPersistedResult = outOfRegionReferralRepository.findByIdAndCrnWithNotes(existingEntity.id, crn)!!
    val sortedNotes: List<OutOfRegionReferralNoteEntity> = oorPersistedResult.notes.sortedByDescending { it.createdAt }
  ...
  fun `should not create a note for oor when oor not found`() {
    restTestClient.post().uri("/cases/$crn/out-of-region-referral/${UUID.randomUUID()}/notes")
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        oorNoteRequestBody(
          note = "Test note",
  ...
  fun `should return OOR timeline when a OOR is created`() {
    val createdOor = restTestClient.post().uri("/cases/{crn}/out-of-region-referral", crn)
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        createOorRequestBody(
          submissionDate = "2026-01-15",
    ...
    restTestClient.get().uri("/cases/{crn}/out-of-region-referral/{id}/timeline", crn, createdOorId)
      .withDeliusUserJwt()
      .exchangeSuccessfully()
      .expectBody<String>()
      .value {
        assertThatJson(it!!).matchesExpectedJson(
          expectedGetOutOfRegionReferralTimelineResponse(
            outOfRegionReferralId = UUID.fromString(createdOorId),
            caseId = case.id,
  ...
  fun `should return OOR timeline when it is created, a note is added, and it is updated twice`() {
    ...
    restTestClient.post().uri("/cases/{crn}/out-of-region-referral/{id}/notes", crn, createdOorId)
      .contentType(MediaType.APPLICATION_JSON)
      .body(oorNoteRequestBody(note = "Test note"))
      .withDeliusUserJwt()
      .exchangeSuccessfully()

    restTestClient.put().uri("/cases/{crn}/out-of-region-referral/{id}", crn, createdOorId)
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        createOorRequestBody(
          submissionDate = "2026-01-15",
    ...
    val createNoteCommitTime = outOfRegionReferralRepository.findByIdAndCrnWithNotes(UUID.fromString(createdOorId), crn)!!
  ...
  fun `should return 404 for timeline when OOR not found`() {
    restTestClient.get().uri("/cases/{crn}/out-of-region-referral/{id}/timeline", crn, UUID.randomUUID())
      .withDeliusUserJwt()
      .exchange()
      .expectStatus().isNotFound
  }
  ...
  fun `should return 404 for timeline when crn does not match`() {
    ...
    restTestClient.get().uri("/cases/{crn}/out-of-region-referral/{id}/timeline", "OTHERCRN", entity.id)
      .withDeliusUserJwt()
      .exchange()
      .expectStatus().isNotFound
  }

  private fun getCommitTimesAsc(outOfRegionReferralId: UUID): List<Instant> {
    val changes = javers.findChanges(
      QueryBuilder.byInstanceId(outOfRegionReferralId, OutOfRegionReferralEntity::class.java).build(),
    )
  ...
  private fun assertPersistedOutOfRegionReferral(
    persistedRecord: OutOfRegionReferralEntity,
  ) {
  ...
  @Test
  fun `should create out of region referral with submission note and return it in the response`() {
  ...
  @Test
  fun `should create out of region referral with blank note and not persist the note`() {
    restTestClient.post().uri("/cases/$crn/out-of-region-referral")
      .contentType(MediaType.APPLICATION_JSON)
      .body(
        createOorRequestBody(
          status = "SUBMITTED",
  ...
  @Test
  fun `should update out of region referral with both submission and outcome notes and return them in the response`() {
    ...
    val updatedRecord = outOfRegionReferralRepository.findByCaseId(case.id)!!
  ...
  @Test
  fun `should update out of region referral with blank note and not overwrite existing outcome note`() {
    val existingEntity = createOor()
  ...
  @Test
  fun `should update submission note on an existing SUBMITTED out of region referral`() {
  ...
  @Test
  fun `should update submission note on a out of region referral that already has an outcome`() {
    val outcomeNoteText = "This is an outcome note"
    val updatedNote = "Updated submission note"
  ...
  private fun createOor(): OutOfRegionReferralEntity {
    val existingEntity = outOfRegionReferralRepository.save(
      buildOutOfRegionReferralEntity(
        caseId = case.id,
        status = EntityOorStatus.SUBMITTED,
